        ...
        public override async Task<TOutput> RunTask(OrchestrationContext context, TInput orchestrationInput)
        {
            ...
            try
            {
                if(context.IsReplaying == false)
                    logger?.LogDebug(100, "OrchestrationBase: '{orchestration}' entered.", this.GetType().FullName);
                ...
                if (context.IsReplaying == false)
                    logger?.LogDebug(101, "OrchestrationBase: '{orchestration}' exited successfully", this.GetType().FullName);
            }
            catch (Exception ex)
            {
                logger?.LogError(102, ex, "Orchestration failed.");
                throw;
            }
